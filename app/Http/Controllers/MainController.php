...
class MainController extends BaseController
{
	...
	protected function setResultError($error, $code = 200)
	{
		...
		if(json_decode($error)!==NULL){
			foreach(json_decode($error) as $e)
				foreach($e as $msg)
					$content[] = $msg;
		}else{
			$content = [$error];
	...
	{
		$this->setDocumentListData([$document]);
	}
	...
	protected function prepareDocumentData(Document $document)
	{
		$data = [
			...
			'created_at' => $document->created_at->format(\DateTime::ISO8601),
			'updated_at' => $document->updated_at->format(\DateTime::ISO8601),
